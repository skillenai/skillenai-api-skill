...
- `net-flow-by-role` (`company_id`) / `net-flow-by-company` (`role_id`) — `arrivals/departures/net_flow` for one company x role cell.
- `role-flows` (`role_id`) / `company-flows` (`company_id`) — the same measures with the OTHER axis collapsed: a role's arrivals/departures across **all employers**, or a company's across **all roles**. This is the supply-side series; do not try to rebuild it by summing `net-flow-by-role` over companies (see below).
- `company-transitions-in/out`, `company-signals`, `company-prestige`, `school-*`, `education-facets`.

**Month grain.** Every dated endpoint above has a `-monthly` twin (`role-flows-monthly`, `net-flow-by-role-monthly`, `role-transitions-in-monthly`, …) exposing `flow_month` / `move_month` alongside the year, so `?flow_year=2024&flow_month=3` is one month rather than March in every year. `GET /v1/talent-graph/endpoints` reports a `grain` of `year`, `month` or `none` per endpoint — cite it, because the two grains cover different populations:

- Month rows are restricted to source dates that state a **month**. Dates giving only a year are excluded rather than imputed to January, so **month rows do not sum to the year row** — use the year endpoint for totals.
- Coverage is about two thirds of dated positions and moves, and it is not random: it rises with recency (roughly half of 1990s starts, about three quarters of 2020s). A month series is thinner and more recency-biased the further back it reaches.
- January still holds ~12% of month-grain volume against 8.3% for a uniform month. That residual is real seasonality, not leftover imputation — January is genuinely the heaviest start month.

**Never sum a company-scoped endpoint to get a role total.** Every flow measure is a distinct-person count, so summing `net-flow-by-role` over companies counts twice anyone who held the same role at two employers in the period — about 3% corpus-wide, but up to 13% for an individual role. `role-flows` counts the person once. (It still counts someone holding two *different* roles in each of them, which is correct for a per-role total.)
...
| `scripts/talent_graph.py` | Talent-graph (supply-side) client: discover endpoints, resolve/fts-expand role families, aggregate role-to-role transitions (feeders/exits) and skill-prevalence across a role family. **`/v1/resolution/entities` caps `limit` at 20** (higher 422s), and 20 is not enough to see a whole family — high-volume members can fall below the cut (`Senior AI Engineer` and `Senior ML Engineer` are both absent from the top-20 expansion of their own families), so probe for missing seniority/synonym variants by exact name and add them. Every dated talent-graph endpoint now has a `-monthly` twin exposing `move_month` / `flow_month`, and `role-flows` / `company-flows` give the all-employer and all-role aggregates — so neither a sub-annual series nor a role-level supply series needs rebuilding from raw profile spells. Check the `grain` field on `/v1/talent-graph/endpoints`, and read the month-grain caveats above before using it: month rows cover only dates that state a month, so they do not sum to the year row. |
...
| `scripts/retime_gif.py` | Re-time an animated GIF losslessly (frames copied in palette mode; only timing metadata changes). Needed because matplotlib's `PillowWriter` **collapses consecutive duplicate frames**, so the usual way to add an end-pause — appending the last frame N times — silently does nothing; a hold must be an explicit per-frame duration. Slow a whole animation with `--factor`, or set absolute `--first-ms`/`--last-ms` holds (scaling a hold with playback speed gives a 16s freeze at 4x). |
...
| `scripts/near_duplicate_scan.py` | "How much of this corpus is a rewrite of itself?" Exact all-pairs cosine over document embeddings (256-dim, L2-normalised, so cosine is a dot product and >100k docs is tractable via chunked matmul). Works on any index carrying embeddings. **Deduplicates by document id and content hash first** — double-ingested documents otherwise surface as "near-duplicate content", which is the easiest way to fabricate this finding. Splits pairs by **author overlap per similarity band**, which separates self-versioning (re-uploads, v2 rewrites) from independent convergence — usually the actual insight. `--dump-pairs` writes sampled pairs per band so you calibrate the threshold by reading them instead of assuming one. `--by-period` computes within-period rates at a fixed subsample size, because a growing corpus mechanically raises nearest-neighbour similarity and a raw trend is an artifact. |
